...
class BaseRLEnv(BaseSimulationEnv, gym.Env):
    def __init__(self, use_gui=True, frame_skip=5, use_visual_obs=False, **renderer_kwargs):
        ...
        self.total_activate_joint_index = [0, 1, 2, 3, 4, 5, 6, 8, 10, 12, 14, 15]
        # self.activate_finger_joint = []
    ...
    def setup(self, robot_name):
        ...
        if self.is_robot_free:
            ...
            velocity_limit = np.array( [1.57] * 3 + [1.0] * 3 + [3.14] * (self.robot.dof - 6))
    ...
    def free_sim_step(self, action: np.ndarray):
        ...
        # #print('----------')
        # #print('before: ', target_qpos)
        x_activate = target_qpos[self.activate_joint_index]
        ...
        target_qpos[self.coupled_joint_index] = np.concatenate((Joint_F, np.array([Joint_B]), (np.array([Joint_C]))))
        # #print('after: ', target_qpos)
        
        # target_qpos_full = np.concatenate((np.zeros(6), target_qpos_finger))
        # #print('0----')
        # #print(target_qvel)
        # #print(target_qpos_full)
        # #print('------')
        # #print(target_qvel)
        # #print(target_qpos)
        # #print('vel:',target_qvel)
        # for joint_index, joint in enumerate(active_joints):
        #     joint.set_drive_property(stiffness=20.0, damping=50.0)
        ...
        for i in range(self.frame_skip):
            # self.robot.set_qpos(target_qpos)
            torque = self.robot.compute_passive_force(external=False, gravity=True, coriolis_and_centrifugal=True)
            
            # thumb_qf = self.PID(target_qpos[-4], self.robot.get_qpos()[-4])
            
            # torque[-4] += thumb_qf
            # #print('torque:', torque)
            # torque[7] = -1
            self.robot.set_qf(torque)
            # #print('get torque', self.robot.get_qf())
            self.scene.step()
        self.current_step += 1

    def PID(self, target_pos, pos):
        kp = 100
        kd = 1
        thumb_qf = kp * (target_pos-pos) 
        return thumb_qf
    ...
    def setup_visual_obs_config(self, config: Dict[str, Dict]):
        ...
        OBS_CONFIG = {
            "relocate": {
                "relocate": {
                    "point_cloud": {
                                    "process_fn": process_relocate_pc, "num_points": 512
                                    }
                             },
                         },
            "relocate_noise": {
                "relocate": {
                    "point_cloud": {
                                    "process_fn": process_relocate_pc_noise, "num_points": 512, "pose_perturb_level": 0.5,"process_fn_kwargs": {
                                                                                                                                                "noise_level": 0.5
                                                                                                                                                }
                                    },
                },
    ...
    def get_visual_observation(self):
        ...
        # #print([camera_obs[i].shape for i in list(camera_obs.keys())],'camera_obs[i].shape')
        # assert len(camera_obs) == 0
    ...
    def get_camera_obs(self):
        ...
        obs_dict = {}
        # print([name for name, camera_cfg in self.camera_infos.items()])
        # assert False
        for name, camera_cfg in self.camera_infos.items():  #only relocate
            cam = self.cameras[name]
            ...
            for i, modality in enumerate(modalities):
                ...
                if modality == "rgb":
                    ...
                    def get_camera_to_robot_pose(self, camera_name)
                        ...
                        camera2robot = self.robot.get_pose().inv() * camera_poseas
    ...
    def get_camera_to_robot_pose(self, camera_name):
        ...
        # gl2sapien = sapien.Pose(q=np.array([0.5, 0.5, -0.5, -0.5]))
        # print('robot pose:', self.robot.get_pose())
    ...
    def observation_space(self):
        ...
        # #print(self.use_visual_obs,'self.use_visual_obs')
        # assert False
